...
int ulp_ddp_reply_cb(const struct nlmsghdr *nlhdr, void *data)
{
	...
	caps_names = global_stringset(ETH_SS_ULP_DDP, nlctx->ethnl2_socket);
	...
	if (tb[ETHTOOL_A_ULP_DDP_STATS]) {
		stat_names = perdev_stringset(nlctx->devname,
			     ...
			     const struct nlattr *const *tb)
{
	...
	cap_names = global_stringset(ETH_SS_ULP_DDP, nlctx->ethnl_socket);
