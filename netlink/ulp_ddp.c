...
static int data_store_stat_cb(const struct nlattr *attr, void *data)
{
	struct ulp_ddp_results *res = data;
	uint16_t type;
	...
	if (type >= res->stat_count)
		return MNL_CB_ERROR;

	res->stat_set[type] = 1;
	res->stats[type] = mnl_attr_get_u64(attr);

	return MNL_CB_OK;
}

int prepare_ulp_ddp_stats(const struct nlattr *nest,
			  const struct stringset *stat_names,
			  struct ulp_ddp_results *res)
{
	int ret;

	res->stat_count = get_count(stat_names);
	res->stats = calloc(res->stat_count, sizeof(*res->stats));
	if (!res->stats) {
		ret = -ENOMEM;
		goto err;
	}

	res->stat_set = calloc(res->stat_count, sizeof(*res->stat_set));
	if (!res->stat_set) {
		ret = -ENOMEM;
		goto err;
	}

	ret = mnl_attr_parse_nested(nest, data_store_stat_cb, res);
	if (ret < 0)
		goto err;
				   ...
				   struct ulp_ddp_results *res)
{
	...
	if (tb[ETHTOOL_A_ULP_DDP_STATS]) {
		ret = prepare_ulp_ddp_stats(tb[ETHTOOL_A_ULP_DDP_STATS],
					    stat_names, res);
		if (ret < 0)
			return ret;
	}
		 ...
		 const struct stringset *stat_names)
{
	...
	ret = prepare_ulp_ddp_results(tb, stat_names, &results);
	if (ret < 0)
		return -EFAULT;
	...
	if (results.stats) {
		...
		for (i = 0; i < results.stat_count; i++) {
			if (!results.stat_set[i])
				continue;

			printf("%s: %lu\n",
			       get_string(stat_names, i),
			       results.stats[i]);
		}
	}

	free(results.stats);
	free(results.stat_set);
	return 0;
}
